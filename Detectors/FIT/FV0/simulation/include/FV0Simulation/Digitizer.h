...
class Digitizer
{
 public:
  ...
  static Float_t PmtResponse(const Float_t* x, const Float_t* par); //fdd
 ...
 private:
  ...
  std::vector<std::vector<Float_t>> mPmtChargeVsTime;       // Charge time series aka analogue signal pulse from PM
  std::vector<Float_t> mPmtChargeVsTimeCfd;                 // Time series for CFD measurement
  UInt_t mNBins;                                            // Number of bins in pulse series
  Float_t mBinSize;                                         // Time width of the pulse bin - HPTDC resolution
  std::unique_ptr<o2::base::CachingTF1> mPmtResponse;       // function which describes the PMT time response
